package fi.gemwars.gameobjects.menu;
...
public class GearPair implements IGameObject {
	...
	public GearPair(float gearsRotationSpeed, Vector2f position, float scale) {
		...
		// Each gear need a own image object, because when making tranforming/rotating
		// images they
		...
		smallGearImage.setCenterOfRotation(smallGearImage.getWidth() / 2 * scale,
				smallGearImage.getHeight() / 2 * scale);
