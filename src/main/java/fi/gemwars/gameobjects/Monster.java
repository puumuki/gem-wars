...
/**
 * Enemies of the player. Stupid ants that move according to a certain pattern
...
public class Monster extends AEntity {
	 ...
	 * <p>
	 * Monsters "hug" the wall on their left. If they encounter a wall, they turn
	 * left until they can get out of the situation. However, if there is no wall,
	...
	private void changeDirection() throws SlickException {
		if (direction == Direction.RIGHT) {
			if (!map.isMonsterColliding(positionX, positionY - 1)
					&& map.isMonsterColliding(positionX - 1, positionY - 1)) {
		...
		if (direction == Direction.UP) {
			if (!map.isMonsterColliding(positionX - 1, positionY)
					&& map.isMonsterColliding(positionX - 1, positionY + 1)) { // can it turn left
				positionX--;
