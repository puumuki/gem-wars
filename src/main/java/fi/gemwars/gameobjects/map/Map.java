...
import fi.gemwars.gameobjects.Direction;
...
/**
 * The Map class is used to save all the data the game needs to draw the map.
 * 
 */
public class Map extends AEntity {
	...
	/**
	 * Pauses the timer when given value is false and restart it when the value is
	 * true.
	 * 
	 * @param active
	 */
	...
	public void render(GameContainer cont, Graphics graph) throws SlickException {
		...
		graph.translate((float) cameraPositionX, (float) cameraPositionY);
	...
	public void update(GameContainer cont, int delta) throws SlickException {
		...
		// FIXME: We are not taking count another player, the camera is centered always
		// to first player
		...
		if (isTimeUp()) {
			...
			/*
			 * for (Player player : players) { if( player.isDead() == false ) {
			 * player.kill(); } }
			 */
		}
	...
	/**
	 * Return starting positions from the map. There can be multiple starting
	 * positions in multiplayer maps.
	 * 
	...
	public boolean isColliding(int x, int y) {
		...
		if (x < 0 || x >= objectLayer.getWidth() || y < 0 || y > objectLayer.getHeight()) {
			return false;
		}
	 ...
	 * gems and other stuff that need to be replaced to their orginal positions
	 * after player dies or a new game is started.
	 */
	public void resetMap() {
		resetPlayers();

		// Problem here is that gems are deleted from memory, game logic should be
		// refactored for this part.
		// Gems should be keeped in memory all the time, they could be moved to deleted
		// gem list or they could be marked
